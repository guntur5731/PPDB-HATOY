...
class PesertaServices
{
  ...
  public function detailPeserta($request)
  {
    ...
    $cekstatus = DB::table('tr_biodata')->where("users", $data->id)->first();
    if ($cekstatus) {
      $data->statusBiodata =  $cekstatus->biodata;
      $data->statusDataKeluarga = $cekstatus->datakeluarga;
      $data->statusAlamat = $cekstatus->alamat;
      $data->statusBerkas = $cekstatus->berkas;
    }
  ...
  public function getPeserta($request)
  {
    ...
    try {
      ...
      $cekstatus = DB::table('tr_biodata')->where("users", $cekToken->id)->first();
      $cekToken->statusBiodata =  0;
      $cekToken->statusDataKeluarga = 0;
      $cekToken->statusAlamat = 0;
      $cekToken->statusBerkas = 0;
      if ($cekstatus) {
        $cekToken->statusBiodata =  $cekstatus->biodata;
        $cekToken->statusDataKeluarga = $cekstatus->datakeluarga;
        $cekToken->statusAlamat = $cekstatus->alamat;
        $cekToken->statusBerkas = $cekstatus->berkas;
      }
  ...
  public function addSertifikat($request)
  {
    $response = new Response;
    try {
      $user = $this->PesertaRepository->cekByUuid($request->userUuid);
      if ($user) {
        $sertifikat = new Sertifikat();
        $sertifikat->keterangan = $request["keterangan"];
        $sertifikat->uuid = Uuid::uuid4()->getHex();
        if ($request["files"] != null) {
          ...
          $kkConvert = $this->decode($request["files"], $request["formatfile"]);
          $fileKK = $folderPath . uniqid() . $request["formatfile"];
          file_put_contents(public_path($fileKK), $kkConvert);
          $sertifikat->files = $fileKK;
        }
        $sertifikat->status = 1;
        $sertifikat->created_by = $user->id;
        $sertifikat->created_at = date('Y-m-d H:i:s');
        $sertifikat->save();
      }
      $response->setCode(200);
      $response->setStatus(true);
      $response->setMessage("Sukses menambah data sertifikat");
    } catch (\Exception $e) {
      $response->setCode(500);
      $response->setMessage($e->getMessage());
    }
  ...
  public function sertifikat($request)
  {
    $response = new Response;
    try {
      $user = $this->PesertaRepository->cekByUuid($request->userUuid);
      if ($user) {
        $serti = Sertifikat::where("created_by", $user->id)->where("status", 1)->get();
        $response->setData($serti);
      }
  ...
  public function deleteSertifikat($request)
  {
    $response = new Response;
    try {
      $serti = Sertifikat::where("id", $request->id)->first();
      if ($serti) {
        $serti->status = 0;
        $serti->save();
      }
      $response->setCode(200);
      $response->setStatus(true);
      $response->setMessage("Sukses delete data");
    } catch (\Exception $e) {
      $response->setCode(500);
      $response->setMessage($e->getMessage());
    }
